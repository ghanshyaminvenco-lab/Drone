...
function getLocalIpAddress() {
    ...
    // Loop through the network interfaces to find the local IPv4 address
    for (let iface in networkInterfaces) {
        ...
        if (ipAddress) break; // Exit once the first non-internal IPv4 address is found
    }

    return ipAddress || 'IP not found';  // Return 'IP not found' if no IP address is found
}

    function fn_displayHelp ()
{
    console.log ("==================================")
    console.log (global.Colors.Bright + "LiveOps Cloud Communication Server version " +  JSON.stringify(v_pjson.version) + global.Colors.Reset);
    console.log ("----------------------------------");
...
{
    console.log ("=============================================")
    console.log (global.Colors.Bright + "LiveOps Communication Server version " +  JSON.stringify(v_pjson.version) + global.Colors.Reset);
    console.log ("---------------------------------------------");
    const localIp = getLocalIpAddress();
    console.log("System IP: " + global.Colors.BSuccess + localIp + global.Colors.Reset);
    console.log ("Server Name  " + global.Colors.BSuccess +  global.m_serverconfig.m_configuration.server_id + global.Colors.Reset);
    console.log ("listening on: " + global.Colors.BSuccess +  global.m_serverconfig.m_configuration.server_ip + global.Colors.Reset + " port: " + global.Colors.BSuccess + global.m_serverconfig.m_configuration.server_port + global.Colors.Reset);
    console.log ("Auth ip: " + global.Colors.BSuccess +  global.m_serverconfig.m_configuration.s2s_ws_target_ip + global.Colors.Reset + " port: " + global.Colors.BSuccess + global.m_serverconfig.m_configuration.s2s_ws_target_port + global.Colors.Reset);
...
function fn_parseArgs()
{
    ...
    if (cmds.hasOwnProperty('v') || cmds.hasOwnProperty('version'))
    {
        ...
        console.log (" Authentication Server version: " + JSON.stringify(v_pjson.version));
